...
export default {
  ...
  computed: {
    displayInventoryList() {
      if (this.searchMode === 'semantic' && this.semanticMatchedList.length > 0) {
        const start = (this.currentPage - 1) * this.pageSize
        const end = start + this.pageSize
        return this.semanticMatchedList.slice(start, end)
      }
      ...
      semanticSearch({ query: this.searchName, topK: 10, model: '' }).then(res => {
        ...
        getInventoryList({}).then(inventoryRes => {
          ...
          if (uniqueMatched.length > 0) {
            ...
            this.total = uniqueMatched.length  // 修复：使用实际搜索结果数量
