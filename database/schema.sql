...
-- 试剂管理系统数据库初始化脚本
-- =============================================
drop database  reagent_management;
-- 创建数据库
...
CREATE TABLE IF NOT EXISTS `storage_incompatibility_rules` (
  ...
  `category_a_id` BIGINT NOT NULL COMMENT '分类A ID',
  `category_b_id` BIGINT NOT NULL COMMENT '分类B ID',
  `incompatibility_level` VARCHAR(20) COMMENT '不兼容等级',
  `reason` TEXT COMMENT '原因',
  `recommendation` TEXT COMMENT '建议',
  `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`),
  KEY `idx_category_a` (`category_a_id`),
  KEY `idx_category_b` (`category_b_id`)
